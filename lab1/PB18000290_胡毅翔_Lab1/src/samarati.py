...
def satisfy(k, vec, MaxSup, Utility_evaluation):
    ...
    if Utility_evaluation == 'Loss_Metric':
        ...
        LM = 0
        for l in range(4):  # attribute
            print(l)
            for j in range(len(tmp)):  # record
                delete = tmp[j]  # get record
                frequency = {}  # frequency
                for i in range(len(t)):  # Traverse
                    x = str(age_Generalization(t[i][0], vec[0]))+str(Generalization(gender_tree, t[i][1], vec[1]))+str(
                        Generalization(race_tree, t[i][2], vec[2]))+str(Generalization(marital_status_tree, t[i][3], vec[3]))
                    if x == delete:
                        s = frequency.get(t[i][l])
                        if s == None:
                            frequency[str(t[i][l])] = 1
                        else:
                            frequency[str(t[i][l])] = s+1
                li = list(frequency.keys())
                for i in range(len(li)):
                    p = frequency.get(li[i])/d.get(tmp[j])
                    LM = LM-p*math.log(p)
        for j in range(len(tmp)):
            if d.get(tmp[j]) < k:
                delete = tmp[j]
